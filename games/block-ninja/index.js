...
const PURPLE = { r: 0xa8, g: 0x55, b: 0xf7 };
const YELLOW = { r: 0xf5, g: 0x9e, b: 0x0b };
const CYAN = { r: 0x22, g: 0xd3, b: 0xee };
const RED = { r: 0xef, g: 0x44, b: 0x44 };
const allColors = [BLUE, GREEN, PINK, ORANGE, PURPLE, YELLOW, CYAN, RED];
...
const generalTargetPool = [];
const generalTargetWireframePool = [];
...
const getTarget = (() => {
	...
	function getTargetOfStyle(color, wireframe) {
		...
		const generalPool = wireframe ? generalTargetWireframePool : generalTargetPool;
		let target = (pool.get(color) || []).pop() || generalPool.pop();
		if (!target) {
			target = new Entity({
	...
	return function getTarget() {
		...
		if (!wireframe && color !== PINK) {
			const randomColor = getRandomColor();
			target.color = randomColor;
			for (let p of target.polys) {
				p.color = randomColor;
				p.strokeColor = colorToHex(randomColor);
				p.strokeColorDark = shadeColor(randomColor, 0.4);
			}
		}
...
const returnTarget = target => {
	...
	const generalPool = target.wireframe ? generalTargetWireframePool : generalTargetPool;
	const colorPool = pool.get(target.color);
	if (colorPool) {
		colorPool.push(target);
	} else {
		generalPool.push(target);
	}
};
...
const generalFragPool = [];
const generalFragWireframePool = [];
...
const createBurst = (() => {
	...
	function getFragForTarget(target) {
		...
		const generalPool = target.wireframe ? generalFragWireframePool : generalFragPool;
		const colorPool = pool.get(target.color);
		let frag = colorPool ? colorPool.pop() : generalPool.pop();
		if (!frag) {
			frag = new Entity({
		...
		for (let p of frag.polys) {
			p.color = target.color;
			p.strokeColor = colorToHex(target.color);
			p.strokeColorDark = shadeColor(target.color, 0.4);
		}
		return frag;
	}
...
const returnFrag = frag => {
	...
	const generalPool = frag.wireframe ? generalFragWireframePool : generalFragPool;
	const colorPool = pool.get(frag.color);
	if (colorPool) {
		colorPool.push(frag);
	} else {
		generalPool.push(frag);
	}
};
...
function resetGame() {
	resetAllTargets();
	while (frags.length) returnFrag(frags.pop());
	state.game.time = 0;
	resetAllCooldowns();
	setScore(0);
	setCubeCount(0);
...
const iconOn = muteBtn.querySelector('.sound-on');
const iconOff = muteBtn.querySelector('.sound-off');

handlePointerDown(muteBtn, () => {
	isMuted = !isMuted;
	if (isMuted) {
...
function tick(width, height, simTime, simSpeed, lag) {
	...
	const forceMultiplier = isMobile ? 1.5 : 1 / (simSpeed * 0.75 + 0.25);
	pointerDelta.x = 0;
	pointerDelta.y = 0;
	pointerDeltaScaled.x = 0;
	pointerDeltaScaled.y = 0;
	...
	for (let i = 0; i < targets.length; i++) {
		...
		const p = entity.polys;
		const sv = entity.shadowVertices;
		const sp = entity.shadowPolys;
		for (let j = 0; j < v.length; j++) allVertices.push(v[j]);
		for (let j = 0; j < p.length; j++) allPolys.push(p[j]);
		for (let j = 0; j < sv.length; j++) allShadowVertices.push(sv[j]);
		for (let j = 0; j < sp.length; j++) allShadowPolys.push(sp[j]);
	}

	for (let i = 0; i < frags.length; i++) {
		const entity = frags[i];
		const v = entity.vertices;
		const p = entity.polys;
		const sv = entity.shadowVertices;
		const sp = entity.shadowPolys;
	...
	for (let i = 0; i < allPolys.length; i++) computePolyNormal(allPolys[i], 'normalWorld');
	for (let i = 0; i < allPolys.length; i++) computePolyDepth(allPolys[i]);
	allPolys.sort((a, b) => b.depth - a.depth);

	
	for (let i = 0; i < allVertices.length; i++) projectVertex(allVertices[i]);
	for (let i = 0; i < allPolys.length; i++) computePolyNormal(allPolys[i], 'normalCamera');
	...
	for (let i = 0; i < allShadowPolys.length; i++) computePolyNormal(allShadowPolys[i], 'normalWorld');
	...
	for (let i = 0; i < allShadowVertices.length; i++) projectVertex(allShadowVertices[i]);
...
function setupCanvas() {
	...
	function handleResize() {
		...
		viewScale = isMobile ? (minDim / 550) : (h / 1000); 
		width = w / viewScale;
		height = h / viewScale;
		canvas.width = w * dpr;
		canvas.height = h * dpr;
